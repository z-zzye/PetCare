...
const VetApplyDetail = () => {
  ...
  const fetchApplyDetail = async () => {
    try {
      ...
      const response = await axios.get(`/admin/vet-applies/${applyId}`);
      setApplyDetail(response.data);
    } catch (err) {
      console.error('수의사 신청 상세 조회 실패:', err);
      setError('수의사 신청 상세 정보를 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      setProcessing(true);
      await axios.patch(`/admin/vet-applies/${applyId}/status`, {
        status: 'APPROVED'
      });
      
      await Swal.fire({
        title: '승인 완료!',
        text: '수의사 신청이 승인되었습니다.',
        icon: 'success',
        timer: 1500,
        showConfirmButton: false
      });
      
      navigate('/admin/vet-apply'); // 목록 페이지로 이동
    } catch (err) {
          ...
          <div className="admin-container">
            <div className="empty-state">
              ...
              <button
                ...
                onClick={() => navigate('/admin/vet-apply')}
              >
                목록으로 돌아가기
              </button>
            </div>
      ...
      <div className="admin-page">
        <div className="admin-container">
          <div className="admin-header">
            <h1 className="admin-title">수의사 신청 상세</h1>
          ...
          <div className="admin-content">
            <div className="vet-apply-detail">
              {/* 기본 정보 섹션 */}
              ...
              {/* 면허 정보 섹션 */}
              <div className="detail-section">
                <h2 className="detail-section-title">면허 정보</h2>
                <div className="detail-grid">
                  <div className="detail-item">
                    <label>수의사 면허번호</label>
                    <span>{applyDetail.licenseNumber}</span>
                  </div>
                  <div className="detail-item">
                    <label>경력 연차</label>
              ...
              {/* 병원 정보 섹션 */}
              <div className="detail-section">
                <h2 className="detail-section-title">병원 정보</h2>
                <div className="detail-grid">
                  <div className="detail-item">
                    <label>소속 병원명</label>
                    <span>{applyDetail.hospitalName}</span>
                  </div>
                  <div className="detail-item">
                    <label>병원 주소</label>
                    <span>{applyDetail.hospitalAddress || '-'}</span>
                  </div>
                  <div className="detail-item">
                    <label>병원 연락처</label>
                    <span>{applyDetail.hospitalPhone || '-'}</span>
                  </div>
                </div>
              </div>
              ...
              {/* 전문 분야 섹션 */}
              <div className="detail-section">
                <h2 className="detail-section-title">전문 분야</h2>
                <div className="detail-content">
                  <div className="detail-item full-width">
                    <label>전문 분야</label>
                    <div className="content-text">
                      {applyDetail.specialization}
                    </div>
                  </div>
                  <div className="detail-item full-width">
                    <label>보유 자격증</label>
                    <div className="content-text">
                      {applyDetail.certifications || '-'}
                    </div>
                  </div>
                </div>
              </div>
