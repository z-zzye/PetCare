      ...
      <div style={{ display: 'flex', maxWidth: 1200, margin: '0 auto', padding: '2rem 0' }}>
        ...
        <div style={{ flex: 2, marginRight: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: 18 }}>
            ...
            <button style={{ marginLeft: 18 }} onClick={handleDeleteSelected}>선택삭제</button>
          </div>
          {cartItems.length === 0 ? (
            ...
            cartItems.map(item => {
              ...
              return (
                <div key={id} style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid #eee', padding: '1.2rem 0' }}>
                  <input type="checkbox" checked={checkedIds.includes(id)} onChange={() => handleCheck(id)} />
                  <img src={
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: '#888', fontSize: '0.98rem', marginBottom: 8 }}>{item.option?.optionName}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <button onClick={() => handleQtyChange(item, item.quantity - 1)} style={{ width: 28, height: 28 }}>-</button>
