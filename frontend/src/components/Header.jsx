  ...
  return (
    <nav className="main-navbar">
      <div className="main-navbar-container">
        ...
        {/* 메뉴 */}
        <ul className="main-navbar-menu">
          {menu.map((item, idx) => (
            <li
              key={item.name}
              className={`main-navbar-item${item.submenu.length ? ' has-dropdown' : ''}${item.disabled ? ' disabled' : ''}`}
        ...
        <div className="main-navbar-right">
          ...
          <button className="main-navbar-icon-btn" title="알림">
            <FaBell size={22} />
            {/* 알림 뱃지 예시 */}
            <span className="main-navbar-badge">3</span>
          </button>
